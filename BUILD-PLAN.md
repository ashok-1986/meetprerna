...
├── DECISIONS.md                       # the locked answers
...
**Week one, in parallel with Phase 1:** book the photographer. See `DECISIONS.md` §15. The build is not the critical path. The photos are.
...
```
Read AGENTS.md and DECISIONS.md first. Work as the Architect persona
from docs/agents.md.

Scaffold the MeetPrerna project.

Stack, locked in DECISIONS.md, do not substitute:
...
They are cut. DECISIONS.md §11 and §12 explain why.
...
2. src/styles/tokens.css with the exact palette and type scale from
   .agents/rules/10-design-system.md. Every value. Nothing invented.
...
```
Read AGENTS.md and DECISIONS.md. Work as the Interface Engineer persona.
No animation in this phase at all.

Build all nine routes with real copy from docs/content.md.

Routes, exactly these, DECISIONS.md §6:
...
1. src/content/portfolio.ts — the typed Piece array (shape in
   DECISIONS.md §20), plus src/content/site.ts for the city line and
   other site-wide strings. No CMS, no Sanity, no database.
   Cloudinary public IDs referenced by string.
...
4. Redirects in next.config.js, the full table from DECISIONS.md §7.
   Verify each with curl -I and paste the output.
5. /consulting: FilloutStandardEmbed, filloutId gvnCVtzfz2us, framed in our own
   chrome per DECISIONS.md §9 D8.2. dynamicResize, inheritParameters, and
...
6. WhatsApp: one small fallback link, footer and beside the /consulting embed
   only. Never a button, never accent-coloured. DECISIONS.md §9.
7. SEO: per-route metadata from content.md §8, sitemap.ts, robots.ts,
   ...
   NO LocalBusiness. NO address. NO map embed. DECISIONS.md §8.
...
```
Read AGENTS.md, DECISIONS.md and .agents/rules/20-motion-performance.md.
Work as the Motion Engineer persona.

Confirm before starting: is 80% of docs/content.md §9 delivered? If not,
stop and say so.

Build only the animations in the motion register, M1 to M13. Nothing else.
An animation without a register row does not get written.
...
```
Read AGENTS.md, DECISIONS.md §12, docs/shaders.md §1 to §4.
...
to a post-launch backlog. The arithmetic is in DECISIONS.md §12.
...
3. Testimonials: only ones naming Prerna, each with a source link.
   DECISIONS.md §13.
4. Analytics firing all six funnel events from DECISIONS.md §2.
...
2. Point DNS
3. curl -I every redirect from DECISIONS.md §7 against the live domain
...
Four defects on the live site right now. Roughly an hour of work. `DECISIONS.md` §14.
...
- If the agent says something contradicts `DECISIONS.md`, believe it and check. That refusal is the ruleset working.
