"use client";

import { useRef, useEffect, useState } from "react";
import { motion, useScroll, useTransform } from "framer-motion";
import Image from "next/image";
import Link from "next/link";

const FEATURED = [
  {
    id: 1,
    category: "Tattoo",
    title: "Permanent stories on skin.",
    description: "Custom tattoos designed from your story — not from a catalogue.",
    image: "/work/featured-tattoo.jpg",
    tags: ["Custom", "Fine Line", "Blackwork", "Geometric"],
  },
  {
    id: 2,
    category: "Illustration",
    title: "Drawn by hand. Felt in the chest.",
    description: "Original illustrations that live on paper, walls, and skin.",
    image: "/work/featured-illustration.jpg",
    tags: ["Original", "Commission", "Hand-drawn", "Narrative"],
  },
  {
    id: 3,
    category: "Vitiligo & Restoration",
    title: "She finds beauty where others looked away.",
    description: "Vitiligo art, scar cover, and tattoo restoration — healing through art.",
    image: "/work/featured-restoration.jpg",
    tags: ["Vitiligo Art", "Restoration", "Scar Cover", "Healing"],
  },
  {
    id: 4,
    category: "Permanent Art",
    title: "Every surface. One restless vision.",
    description: "Painting, wall art, permanent makeup — art in every form.",
    image: "/work/featured-permanent.jpg",
    tags: ["Painting", "Wall Art", "Permanent Makeup", "Mixed Media"],
  },
];

const CARD_WIDTH = "clamp(320px, 38vw, 520px)";
const CARD_GAP = 24;

export default function FeaturedWork() {
  const scrollRef = useRef<HTMLDivElement>(null);
  const [scrollProgress, setScrollProgress] = useState(0);
  const [isScrolling, setIsScrolling] = useState(false);

  // Track scroll progress for indicators
  useEffect(() => {
    const el = scrollRef.current;
    if (!el) return;

    const updateProgress = () => {
      const maxScroll = el.scrollWidth - el.clientWidth;
      setScrollProgress(maxScroll > 0 ? el.scrollLeft / maxScroll : 0);
    };

    el.addEventListener("scroll", updateProgress, { passive: true });
    updateProgress();

    return () => el.removeEventListener("scroll", updateProgress);
  }, []);

  const scrollLeft = () => {
    const el = scrollRef.current;
    if (el) el.scrollBy({ left: -(320 + CARD_GAP), behavior: "smooth" });
  };

  const scrollRight = () => {
    const el = scrollRef.current;
    if (el) el.scrollBy({ left: 320 + CARD_GAP, behavior: "smooth" });
  };

  return (
    <section
      style={{
        background: "#0D0D0D",
        padding: "120px 0 100px",
        overflow: "hidden",
      }}
    >
      {/* ── Header ── */}
      <div
        style={{
          display: "flex",
          justifyContent: "space-between",
          alignItems: "flex-end",
          padding: "0 64px",
          marginBottom: "48px",
        }}
      >
        <motion.p
          initial={{ opacity: 0, y: 16 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1] }}
          style={{
            fontFamily: "Lato, sans-serif",
            fontSize: "0.65rem",
            letterSpacing: "0.25em",
            color: "#C4FF61",
            textTransform: "uppercase",
            opacity: 0.8,
            margin: 0,
          }}
        >
          · Featured Work · {FEATURED.length} categories
        </motion.p>

        <div style={{ display: "flex", alignItems: "center", gap: "24px" }}>
          {/* Arrow controls */}
          <div style={{ display: "flex", gap: "8px" }}>
            <button
              onClick={scrollLeft}
              aria-label="Scroll left"
              disabled={scrollProgress <= 0.01}
              style={{
                width: "40px",
                height: "40px",
                borderRadius: "50%",
                border: "1px solid rgba(253,255,233,0.15)",
                background: "transparent",
                color: "rgba(253,255,233,0.5)",
                cursor: "pointer",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                transition: "border-color 0.2s, color 0.2s, opacity 0.2s",
                opacity: scrollProgress <= 0.01 ? 0.4 : 1,
              }}
            >
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                <path d="M9 2L4 7L9 12" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </button>
            <button
              onClick={scrollRight}
              aria-label="Scroll right"
              disabled={scrollProgress >= 0.99}
              style={{
                width: "40px",
                height: "40px",
                borderRadius: "50%",
                border: "1px solid rgba(253,255,233,0.15)",
                background: "transparent",
                color: "rgba(253,255,233,0.5)",
                cursor: "pointer",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                transition: "border-color 0.2s, color 0.2s, opacity 0.2s",
                opacity: scrollProgress >= 0.99 ? 0.4 : 1,
              }}
            >
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                <path d="M5 2L10 7L5 12" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </button>
          </div>

          <motion.div
            initial={{ opacity: 0 }}
            whileInView={{ opacity: 1 }}
            viewport={{ once: true }}
            transition={{ delay: 0.2, duration: 0.7 }}
          >
            <Link
              href="/work"
              style={{
                fontFamily: "Lato, sans-serif",
                fontSize: "0.65rem",
                letterSpacing: "0.2em",
                color: "rgba(253,255,233,0.35)",
                textDecoration: "none",
                textTransform: "uppercase",
                transition: "color 0.3s",
              }}
            >
              All work →
            </Link>
          </motion.div>
        </div>
      </div>

      {/* ── Horizontal Scroll Track ── */}
      <div
        ref={scrollRef}
        style={{
          display: "flex",
          flexDirection: "row",
          gap: CARD_GAP,
          overflowX: "auto",
          overflowY: "hidden",
          scrollSnapType: "x mandatory",
          WebkitOverflowScrolling: "touch",
          paddingLeft: "64px",
          paddingRight: "64px",
          paddingBottom: "16px",
          scrollbarWidth: "none",
          msOverflowStyle: "none",
        }}
      >
        {FEATURED.map((item, i) => (
          <FeaturedCard key={item.id} item={item} index={i} />
        ))}

        {/* Scroll progress indicator - hidden last card spacer */}
        <div
          style={{
            width: "100%",
            maxWidth: "1px",
            flexShrink: 0,
          }}
        />
      </div>

      {/* ── Scroll Progress Indicator ── */}
      <div
        style={{
          position: "relative",
          height: "4px",
          marginTop: "16px",
          padding: "0 64px",
          background: "rgba(253,255,233,0.08)",
          borderRadius: "2px",
          overflow: "hidden",
        }}
      >
        <motion.div
          style={{
            height: "100%",
            width: "25%",
            background: "linear-gradient(90deg, #C4FF61, #EAFF27)",
            borderRadius: "2px",
            transformOrigin: "left center",
            willChange: "transform",
          }}
          animate={{ x: `${scrollProgress * 300}%` }}
          transition={{ duration: 0.1, ease: "linear" }}
        />
      </div>

      {/* ── Footer Links ── */}
      <div
        style={{
          padding: "48px 64px 0",
          display: "flex",
          justifyContent: "space-between",
          alignItems: "flex-start",
          gap: "48px",
        }}
      >
        <motion.div
          initial={{ opacity: 0, y: 16 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ delay: 0.3, duration: 0.7, ease: [0.16, 1, 0.3, 1] }}
          style={{ flex: 1 }}
        >
          <p
            style={{
              fontFamily: "'Times New Roman', Times, serif",
              fontSize: "0.95rem",
              fontStyle: "italic",
              color: "rgba(253,255,233,0.2)",
              margin: "0 0 16px",
              lineHeight: 1.6,
            }}
          >
            She has lived in three cities and left her mark on all of them.
          </p>
          <Link
            href="/story"
            style={{
              fontFamily: "Lato, sans-serif",
              fontSize: "0.65rem",
              letterSpacing: "0.2em",
              color: "rgba(253,255,233,0.35)",
              textDecoration: "none",
              textTransform: "uppercase",
              transition: "color 0.3s",
              display: "inline-block",
            }}
            onMouseEnter={(e) => (e.currentTarget.style.color = "#C4FF61")}
            onMouseLeave={(e) => (e.currentTarget.style.color = "rgba(253,255,233,0.35)")}
          >
            Read how she got here →
          </Link>
        </motion.div>

        <motion.div
          initial={{ opacity: 0, y: 16 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ delay: 0.4, duration: 0.7, ease: [0.16, 1, 0.3, 1] }}
          style={{ flexShrink: 0 }}
        >
          <Link
            href="/work"
            style={{
              fontFamily: "Lato, sans-serif",
              fontSize: "0.7rem",
              letterSpacing: "0.2em",
              color: "rgba(253,255,233,0.4)",
              textDecoration: "none",
              textTransform: "uppercase",
              display: "inline-flex",
              alignItems: "center",
              gap: "12px",
              border: "1px solid rgba(253,255,233,0.1)",
              padding: "16px 36px",
              borderRadius: "100px",
              transition: "border-color 0.3s, color 0.3s, background 0.3s",
            }}
            onMouseEnter={(e) => {
              e.currentTarget.style.borderColor = "rgba(253,255,233,0.4)";
              e.currentTarget.style.color = "#FDFFE9";
              e.currentTarget.style.background = "rgba(253,255,233,0.05)";
            }}
            onMouseLeave={(e) => {
              e.currentTarget.style.borderColor = "rgba(253,255,233,0.1)";
              e.currentTarget.style.color = "rgba(253,255,233,0.4)";
              e.currentTarget.style.background = "transparent";
            }}
          >
            See all work
          </Link>
        </motion.div>
      </div>
    </section>
  );
}

function FeaturedCard({
  item,
  index,
}: {
  item: (typeof FEATURED)[0];
  index: number;
}) {
  const [hovered, setHovered] = useState(false);

  return (
    <motion.div
      initial={{ opacity: 0, x: 40 }}
      whileInView={{ opacity: 1, x: 0 }}
      viewport={{ once: true, margin: "-100px" }}
      transition={{
        duration: 0.9,
        delay: index * 0.08,
        ease: [0.16, 1, 0.3, 1],
      }}
      onHoverStart={() => setHovered(true)}
      onHoverEnd={() => setHovered(false)}
      style={{
        position: "relative",
        flexShrink: 0,
        width: CARD_WIDTH,
        height: "60vh",
        minHeight: "400px",
        overflow: "hidden",
        scrollSnapAlign: "start",
        borderRadius: "2px",
        cursor: "pointer",
      }}
    >
      <Link href="/work" style={{ display: "block", height: "100%", textDecoration: "none" }}>
        {/* Image with hover zoom */}
        <motion.div
          style={{ position: "absolute", inset: 0, zIndex: 1 }}
          animate={{ scale: hovered ? 1.04 : 1 }}
          transition={{ duration: 0.9, ease: [0.16, 1, 0.3, 1] }}
        >
          <Image
            src={item.image}
            alt={item.category}
            fill
            sizes="(max-width: 768px) 90vw, 40vw"
            priority={index === 0}
            style={{
              objectFit: "cover",
              objectPosition: "center",
              filter: hovered
                ? "grayscale(0%) brightness(0.7)"
                : "grayscale(25%) brightness(0.5)",
              transition: "filter 0.9s ease",
            }}
          />
        </motion.div>

        {/* Gradient overlay - darker at bottom for text */}
        <div
          style={{
            position: "absolute",
            inset: 0,
            zIndex: 2,
            background:
              "linear-gradient(to top, rgba(13,13,13,0.95) 0%, rgba(13,13,13,0.3) 50%, transparent 100%)",
            pointerEvents: "none",
          }}
        />

        {/* Subtle vignette on hover */}
        <motion.div
          animate={{
            opacity: hovered ? 1 : 0,
          }}
          transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }}
          style={{
            position: "absolute",
            inset: 0,
            zIndex: 3,
            pointerEvents: "none",
            background:
              "radial-gradient(ellipse at center, transparent 50%, rgba(196,255,97,0.08) 100%)",
          }}
        />

        {/* Card content — bottom */}
        <div
          style={{
            position: "absolute",
            bottom: "32px",
            left: "28px",
            right: "28px",
            zIndex: 4,
          }}
        >
          <p
            style={{
              fontFamily: "Lato, sans-serif",
              fontSize: "0.6rem",
              letterSpacing: "0.25em",
              color: "#C4FF61",
              textTransform: "uppercase",
              margin: "0 0 10px",
              opacity: 0.9,
            }}
          >
            {item.category}
          </p>
          <motion.h3
            initial={{ opacity: 0, y: 12 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ delay: 0.2, duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
            style={{
              fontFamily: "'Times New Roman', Times, serif",
              fontSize: "clamp(1.2rem, 2.2vw, 1.8rem)",
              fontWeight: 700,
              fontStyle: "italic",
              color: "#FDFFE9",
              margin: "0 0 14px",
              lineHeight: 1.15,
            }}
          >
            {item.title}
          </motion.h3>

          <motion.p
            initial={{ opacity: 0, y: 12 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ delay: 0.3, duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
            style={{
              fontFamily: "Lato, sans-serif",
              fontSize: "0.85rem",
              color: "rgba(253,255,233,0.55)",
              margin: "0 0 18px",
              lineHeight: 1.6,
              maxWidth: "90%",
            }}
          >
            {item.description}
          </motion.p>

          {/* Tags */}
          <motion.div
            initial={{ opacity: 0 }}
            whileInView={{ opacity: 1 }}
            viewport={{ once: true }}
            transition={{ delay: 0.4, duration: 0.5 }}
            style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}
          >
            {item.tags.map((tag) => (
              <span
                key={tag}
                style={{
                  fontFamily: "Lato, sans-serif",
                  fontSize: "0.55rem",
                  letterSpacing: "0.12em",
                  color: "rgba(253,255,233,0.45)",
                  border: "1px solid rgba(253,255,233,0.12)",
                  padding: "4px 12px",
                  borderRadius: "100px",
                  textTransform: "uppercase",
                  transition: "border-color 0.2s, color 0.2s, background 0.2s",
                }}
                onMouseEnter={(e) => {
                  e.currentTarget.style.borderColor = "#C4FF61";
                  e.currentTarget.style.color = "#C4FF61";
                  e.currentTarget.style.background = "rgba(196,255,97,0.1)";
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.borderColor = "rgba(253,255,233,0.12)";
                  e.currentTarget.style.color = "rgba(253,255,233,0.45)";
                  e.currentTarget.style.background = "transparent";
                }}
              >
                {tag}
              </span>
            ))}
          </motion.div>
        </div>

        {/* Hover arrow - top right */}
        <motion.div
          animate={{
            opacity: hovered ? 1 : 0,
            x: hovered ? 0 : 12,
            scale: hovered ? 1 : 0.9,
          }}
          transition={{ duration: 0.35, ease: [0.16, 1, 0.3, 1] }}
          style={{
            position: "absolute",
            top: "24px",
            right: "24px",
            width: "40px",
            height: "40px",
            borderRadius: "50%",
            border: "1px solid #C4FF61",
            display: "flex",
            alignItems: "center",
            justifyContent: "center",
            zIndex: 5,
            background: "rgba(13,13,13,0.6)",
            backdropFilter: "blur(4px)",
          }}
        >
          <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
            <path
              d="M2 10L10 2M10 2H4M10 2V8"
              stroke="#C4FF61"
              strokeWidth="1.5"
              strokeLinecap="round"
              strokeLinejoin="round"
            />
          </svg>
        </motion.div>
      </Link>
    </motion.div>
  );
}