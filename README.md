...
| `PRD.md` (592) | `PRD.md` | Kept. Business doc. §4, §6.1, §7.2 superseded. |
...
| `animations.md` (542) + `shaders.md` (659) | *(retired)* | Motion governance (`MOTION.md`) has since been retired entirely — colour/font/layout content was folded into `DESIGN.md`, motion content was not carried forward anywhere. |
...
├── DESIGN.md              # colour, font, and layout rules — the sole design-system reference
...
└── .agents/
    ├── rules/             # 00-core, 30-quality
    └── workflows/         # /build-section, /review-ui
```

`DECISIONS.md`, `MOTION.md`, `10-design-system.md` and `20-motion-performance.md` have all been retired. Delete after copying across: `designs.md`, `animations.md`, `shaders.md`, `INDEX.md`.

---

## Precedence

`AGENTS.md` > `BLUEPRINT.md` > `ARCHITECTURE.md` > `PRD.md` > `docs/agents.md` > `DESIGN.md` > `docs/content.md` > `docs/components.md`

**`PRD.md` §5 is dead.** `BLUEPRINT.md` replaces it.

If a doc contradicts `DESIGN.md` on colour, font, or layout, the doc is wrong. Fix the doc, not the spec.

---

## Reading order

**30 minutes, to start building:** `AGENTS.md`, `DESIGN.md`, your persona in `docs/agents.md`, then `BUILD-PLAN.md` Phase 1.

**Everything else is read on demand.** `DESIGN.md` when you touch visuals. `docs/content.md` when you write copy. `docs/components.md` when you need a component API. Nobody reads all eight in one sitting, including the agent.
...
1. **All launch assets are in hand.** The content gate is lifted. Replaced by a one-day asset audit in Phase 1 covering counts, resolution, and written subject consent.
2. **The neon is the brand colour and it stays.** Pillars are expressed through quantity, placement and pacing, not hue. See the accent budget, `DESIGN.md` §1. Marigold is still deleted because two neons mean neither one signals action.
3. **No owned studio.** Collaborates with partner studios across Mumbai and Navi Mumbai, plus travel. New "Where to find me" section. `Person` plus `Service` schema, no `LocalBusiness`.
4. **Alza is Prerna's stage name.** Testimonials are legitimate. Disclose once on `/about`, add `alternateName` to schema, keep source links.
...
**Two shaders ship, ambient only.** The site is carried by GSAP choreography, typography and one silent looping clip of her hand working. That last one will do more for the Meditation pillar than every shader in the original spec, weighs about 400KB, and works on a four-year-old Android.
...
| Live site defects | 3 open, roughly one hour. |
